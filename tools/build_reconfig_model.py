...
# rear-centre LiDAR -> rear switch: straight in along z, one drop (order z, y)
mat_rl = unlit(color=[0.10, 0.42, 0.95, 1.0], name="RearLidar-line")
add_ortho((0.0, 5.5, -18.5), (0.0, 4.6, REAR_NEW_Z - 1.98), 0.16, mat_rl, order=(2, 1, 0))
...
# LiDAR -> switch: right-angle but minimal - drop to the switch's height, then run
# straight in along z to the box (order y, x, z). Front-centre binds to ONE (A).
mat_fll = unlit(color=[0.10, 0.42, 0.95, 1.0], name="FLidar-line")
FTOP = 4.2 + 2.4 / 2   # front box top height
add_ortho((-8.5, 10.0, 16.2), (FBx, FTOP, FZ), 0.16, mat_fll, order=(1, 0, 2))  # FL -> B
add_ortho((8.3, 10.0, 16.2), (FAx, FTOP, FZ), 0.16, mat_fll, order=(1, 0, 2))   # FR -> A
